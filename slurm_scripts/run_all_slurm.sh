...
# Define arrays
models=(Falcon-7b Falcon-40b Llama3.1-8b Llama3.1-70b Llama-7b Llama-13b Llama-70b Mistral Mixtral Solar Yi-6b Yi-34b)
      ...
      for prompt_phrasing in "${prompt_phrasings[@]}"; do
        ...
        if [ ! -f "$filename" ]; then
          ...
          if [ "$model" = "Llama3.1-70b" ]; then
            sbatch single_slurm_big.sh "$model" "$dataset" "$question_range" "$prompt_phrasing"
          else
