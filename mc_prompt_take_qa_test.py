...
class Test(object):
    ...
    def make_prompt(self, choices, question_string):
        choice_str = 'A. ' + ' '.join([f'or {ascii_uppercase[i+1]}.' for i in range(len(choices))])
        return f"""Below is a multiple-choice question. Choose the letter which best answers the question. Keep your response as brief as possible: simply state {choice_str}
...
"""
    ...
    def run_test(self, start_q, end_q):
        ...
        for i in range(start_q, end_q):
            ...
            prompt = self.make_prompt(choices_for_q, question_string)
